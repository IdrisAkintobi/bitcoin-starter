...
export class BIP32AddressGenerator {
    ...
    generateAddress(addressType: string, index = 0) {
        const childKey = this.rootKey.derivePath(`m/44'/0'/0'/0/${index}'`);
        let address;

        switch (addressType) {
            case AddressType.P2PK:
                const obj = payments.p2pk({ pubkey: this.keyPair.publicKey });
                return { address: obj.pubkey?.toString('hex') };
            case AddressType.P2PKH:
                return payments.p2pkh({ pubkey: childKey.publicKey });
            case AddressType.P2SH:
                return payments.p2sh({
                    redeem: payments.p2wpkh({ pubkey: childKey.publicKey }),
                });
            case AddressType.P2WPKH:
                return payments.p2wpkh({ pubkey: childKey.publicKey });
            case AddressType.P2WSH:
                return payments.p2wsh({
                    redeem: payments.p2wpkh({ pubkey: childKey.publicKey }),
                });
            case AddressType.P2TR:
                return payments.p2tr({ pubkey: this.keyPair.publicKey });
            default:
